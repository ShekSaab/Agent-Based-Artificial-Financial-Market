...
LAMBDA_INV =  0.01
LAMBDA_HOLD = 0.05
LAMBDA_FUNDAMENTAL = 0.1        # Weight for the "Value Investing" bonus
...
def run_sim(seed):
    ...
    for t in range(1, TE+1):
        ...
        if is_rl:
            ...
            # --- QUESTION 4 MODIFICATION: Fundamental Value Reward ---
            # Calculate a bonus based on the INTENT of the order relative to Pf.
            fund_reward = 0.0
            
            # If Action is Buy (Market Order=0 or Limit Order=2)
            if a == 0 or a == 2:
                # We want to Buy CHEAP (Price < Pf)
                # Note: 'price' variable holds the limit price calculated above in the code
                if price is not None:
                     fund_reward = LAMBDA_FUNDAMENTAL * (Pf - price)

            # If Action is Sell (Market Order=1 or Limit Order=3)
            elif a == 1 or a == 3:
                # We want to Sell EXPENSIVE (Price > Pf)
                if price is not None:
                    fund_reward = LAMBDA_FUNDAMENTAL * (price - Pf)
            
            # Total Reward = PnL Change - Inv Penalty + Fundamental Bonus
            r = (pnl_after - pnl_before) - LAMBDA_INV * abs(rl_portfolios[trader_id].inv) + fund_reward
            # ---------------------------------------------------------
